...
void modified_I18_branch_myasm(std::vector<int64_t> & positions, std::vector<ssp_t> primes, unsigned char * S, int logI, unsigned int N) /*{{{*/
{
    SMALLSIEVE_COMMON_DEFS();
    ASSERT_ALWAYS(positions.size() == primes.size());
    for(auto const & ssp : primes) {
        ...
        asm(
                "leal (%[p], %[p]), %%ebx\n"    // p+p
                "movl %%ebx, %%edx\n"
                // see gcc doc 6.45.2.8 x86 Operand Modifiers
                "xorl %k[p], %%edx\n"           // (p+p)^p
                "movl %[j0], %%eax\n"           // j0
                "movl $0x01, %%ecx\n"
                "andl %%eax, %%ecx\n"           // (j0&1)
                "movl %[j1], %%ecx\n"           // j1
                "cmovnzl %k[p], %%ebx\n"        // p if j0 odd, 2p otherwise
                // ebx=px ecx=j1 eax=j edx=spx
                "cmpl %%eax, %%ecx\n"   
                "jbe 1f\n"
                //XXX                "testl $0x01, %%eax\n"
                //XXX             "jnz 4f\n"
                ...
                "leal (%[pos],%[p]), %k[pi]\n"   // pos+p
                "movl %[ii0], %%r13d\n"         // si.conf.sublat.i0
                "xorl %[pos], %%r13d\n"         // si.conf.sublat.i0^pos
                "orl %%eax, %%r13d\n"           // j|(si.conf.sublat.i0^pos)
                "testl $0x01, %%r13d\n"
                "cmovnel %[pos], %k[pi]\n"       // pos or pos+p
                "movb %[logp], %%r13b\n"        // logp
                "movl %[i1_minus_i0], %%r11d\n"
                "leaq (%[S0], %%r11), %%r11\n"  // S1 = S0 + (i1 - i0)
                "leaq (%[S0], %[pi]), %[pi]\n"  // pi
                // sieve a full line.
                "cmp %[pi], %%r11\n"
            ...
            [pi]"=&r"(overrun)
                ...
                [p]"r"(p),
                "1"(S0),
                [i1_minus_i0]"g"((int)(i1-i0)),
                [I]"g"((uint64_t)I),// if memory, we'll read a quad word...
                [r]"g"((uint32_t)r),
                [ii0]"g"((uint32_t)si.conf.sublat.i0),
                [j0]"g"((uint32_t)j0),
                [j1]"g"((uint32_t)j1),
                [logp]"m"((uint8_t)logp)
                         : "ax", "bx", "cx", "dx", "r11", "r13", "memory"
                             );
        if (logI > LOG_BUCKET_REGION) {
            int N1 = N + interleaving;
            int Q = logI - LOG_BUCKET_REGION;
...
/* some tests
 * chocolat ~/NFS/cado $ g++ -W -Wall -I./build/chocolat -std=c++11 bench-smallsieve.cpp -g  -O3 -DNDEBUG -lgmp && for i in {2..15} ; do echo -n "$i " ; ./a.out -bmin $i -bmax $((i+1)) -A 33 | tail -n +2 | cut -c28-| xargs echo ; done
 * the star indices that we have an inconsistency for modified_I18_branch_myasm
 *
 * order of functions tested:
    modified_I18_branch_C,
    current_I18_branch,
    legacy_branch,
    modified_I18_branch_myasm,
 *
2 0.914 0.952 0.951 1.551
3 0.523 0.528 0.519 0.789 *
4 0.799 0.799 0.795 0.955
5 0.811 0.813 0.808 0.820 *
6 0.791 0.830 0.823 0.844 *
7 0.508 0.493 0.491 0.508 *
8 0.458 0.459 0.454 0.484 *
9 0.383 0.385 0.369 0.441 *
10 0.387 0.398 0.361 0.472 *
11 0.479 0.498 0.404 0.550 *
12 0.675 0.703 0.558 0.714 *
13 1.086 1.091 0.808 1.014
14 1.652 1.757 1.258 1.623
15 3.149 3.165 2.296 3.071
 */
