...
# R=GF(2)['t,x']
# OUTPUT: a vector "cancelation" such that cancelation[k]=probability that
# the cancelation (gap) is exactly k
# Definition: cancelation (aka. gap) is the decrease in degree of F(a,b)
# when compared to the expected degree of F(a,b) which is
# max_i(deg(fi)+i*deg(a)+(d-i)*deg(b)).
...
def truncated_root_to_statistics(r,prec,A,B,At,lost_prec,Am=0,Bm=0):
    ...
    lbdb=max(Bm,nb_conditions-delta_degree-1,Am-delta_degree) 
    #result=sum([2^(i+(i+delta_degree+1-nb_conditions)) for i in [lbdb..ubdb]])/((2^(A+1)-2^(Am))*(2^(B+1)-2^(Bm)))
    result=2^(delta_degree+1-nb_conditions+2*lbdb).n()*(4^(ubdb-lbdb+1)-1).n()/3/((2^(A+1)-2^(Am)).n()*(2^(B+1)-2^(Bm))).n()
    return result


def truncated_root_to_alpha(r,prec,lost_prec,skewness,q=2):
    r1,r2=At(r[0]),At(r[1])
    if r1 != 0:
        delta_degree=At(r1).degree()
        nb_conditions=prec-lost_prec
        ...
        if r2 != 0:
            ...
            delta_degree=-prec
            nb_conditions=0
    return (q-1)/(q+1)*q^-abs(skewness-delta_degree)*q^-max(0,nb_conditions-1)
...
def real_roots_stats(f,max_prec,A,B,R,Am=0,Bm=0):
    # Am, Bm = min degrees of a resp. b
    ...
    for i in range(len(truncroots)):
        ...
        truncroots[i][4]=truncated_root_to_statistics(r,prec,A,B,At,lost_prec,Am,Bm).numerical_approx()
    #print truncroots
    cancelation=tree_to_cancelation(truncroots,N)
    return cancelation


def alpha_infty(f,skewness,max_prec=5):
    F=R.base_ring()
    q=F.cardinality()
    t,x=R.gens()
    At.<t>=F['t']
    ...
    for i in range(len(truncroots)):
        r_tilde=truncroots[i][0]
        prec=truncroots[i][2]
        gap=r_tilde_to_gap(r_tilde,prec,f_bar,s,R)
        r=root_of_reversed_poly_to_real_root(s,r_tilde,R)
        lost_prec=valuation(At(r_tilde),At(t))
        if gap > 0:
            result-=truncated_root_to_alpha(r,prec,lost_prec,skewness,q)
    return result
