                            ...
                            weight_t *ideals_weight)
{
  ...
  else
  {
    ...
    my_tmp = (ideal_merge_t *) malloc(my_br->nb_above_min_index * sizeof (ideal_merge_t));
 ...
 */
inline uint32_t
insert_relation_in_dup_hashtable (uint32_t *H, unsigned long K, buf_rel_t *rel, 
                                  double *cost, unsigned int *is_dup)
{
  uint64_t h;
  uint32_t i, j;
...
inline void 
compute_index_rel (renumber_t tab, buf_rel_t *rel)
{
  unsigned int i;
  p_r_values_t r;
