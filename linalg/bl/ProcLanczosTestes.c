...
// To All Processes
	...
	T2 = wallclock_microseconds();
	SumTimeSD += (T2 - T1);

	TSMultDmatrixBit(SizeABlock, n, N, a, ResmN_Dist, AVi_Dist,
			 p * (m / size), SizeABlock);

	T3 = wallclock_microseconds();
	SumTimeTSD += (T3 - T2);
...
// end construct X0
	    ...
	    T5 = wallclock_microseconds();
	    SumTimeOper += (T5 - T4);

	    printf
		("Step= %lu      SizeWi=  %lu    SumRank= %lu    Machine= %d\n",
		 Step, SizeS[0], SumRank, p);
...
"Small" Linear algebra to get a 64 block of vectors in the Kernel
...
// Small linear algebra to compute the  elements of the Kernel 
    ...
    Kernel.Ncols = Block;
    KernelSparse(M.Data, Result, M.Nrows, M.Ncols, Block, Kernel.Data);
