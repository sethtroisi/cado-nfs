...
unsigned int read_hexstring_u64(std::istream& is, uint64_t * ptr, unsigned int n)
{
    unsigned int i = 0;
    unsigned int ir = 0;
    uint64_t x = 0UL;
...
unsigned int write_hexstring_u64(std::ostream& o, const uint64_t * ptr, unsigned int n)
{
    const char hxits[] = "0123456789abcdef";
    unsigned int i = 0;
    for( ; i < n ; ) {
        uint64_t v = *ptr++;
        for(unsigned int ir = 0 ; ir < 64 && i < n ; ) {
            o << hxits[v&0x0f];
            if (!o) return i;
            v >>= 4;
            ir += 4;
