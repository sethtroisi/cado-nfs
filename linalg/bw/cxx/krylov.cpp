...
namespace globals {
    ...
    /* only meaningful for threads */
    vector<matrix_slice> slices;
...
void fill_check_vector(unsigned int i0, unsigned int i1,
        int32_t * p,
...
bool display_decision(int n, int p)
{
    ...
    for (unsigned int i = 0; i < sizeof(per) / sizeof(per[0]); i++) {
	if (n > p + per[i])
	    continue;
...
struct thread:public traits {
    ...
    thread(void *ptr) {
        ...
        using globals::slices;

        globals::vdata < traits > *vptr = (globals::vdata < traits > *)ptr;

        t = tseqid();

        BUG_ON(thread_class_ptr.size() <= (unsigned int) t);
    ...
    void flip_flap(scalar_t * dst, scalar_t * src) {
        ...
        for (int x = 0; x < mine.nt; x++) {
            if (x == t)
                continue;
            unsigned int xi0 = (x * nr) / mine.nt;
            unsigned int xi1 = ((x + 1) * nr) / mine.nt;
...
struct krylov_thread:public thread < traits, krylov_thread < traits > > {
    ...
    void use_vector(scalar_t * dst) {
        ...
        for (unsigned int x = 0; x < px.size(); x++) {
            ...
            if (px[x].size() == 1) {
                traits::print(a[x], dst[px[x][0]]) << "\n";
            } else {
                wide_scalar_t sum_of_mxis;
                scalar_t reduced;
                zero(sum_of_mxis);
                for(unsigned int s = 0 ; s < px[x].size() ; s++) {
                    addmul(sum_of_mxis, dst[px[x][s]], 1);
                }
                reduce(reduced, sum_of_mxis);
                traits::print(a[x], reduced) << "\n";
            }
        }
        ...
        unsigned int r = super::done / cp_lag;
...
struct mksol_thread:public thread < traits, mksol_thread < traits > > {
    ...
    void flush(bool priv = false) {
        ...
        if (rc == 1) {
            ...
            mpz_class z;
            for (int x = 0; x < mine.nt; x++) {
                unsigned int xi0 = (x * globals::nr) / mine.nt;
                unsigned int xi1 = ((x + 1) * globals::nr) / mine.nt;
                const self *xptr = (const self *)
                    globals::thread_class_ptr[x];
                for (unsigned int xi = xi0; xi < xi1; xi++) {
...
unsigned int set_f_coeffs(typename traits::scalar_t * f, unsigned int c,  unsigned int maxdeg)
{
    ...
    for (deg = 0; !fx.eof(); deg++) {
         ...
         */
        int nonzero = 0;
        for (unsigned int i = 0; i < globals::n; i++) {
            mpz_class z = *inp++;
            *foo++ = z;
            nonzero += z != 0;
        }
...
int main(int argc, char *argv[])
{
    ...
    // We start by getting m and n -- quick and easy.
    //
    config_file_t cf;
    ...
    ASSERT_ALWAYS((unsigned int) m == bw_x.size());

    // accord this with the i0,j0 i1,j1 command line arguments.
    ...
    // Now we go reading the matrix. We use the auxiliary code for that.
    matrix_stats stats;
    std::ifstream mtx;
    must_open(mtx, files::matrix);

    slices.assign(mine.nt, matrix_slice());

    // we'll need to put coefficients somewhere.
    stats.need_slices(&slices);

    stats(mtx);

    // populate global variables with the results.
    nr = stats.nr;
    nc = stats.nc;
