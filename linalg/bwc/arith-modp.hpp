...
template<int _extra, typename T>
    struct gfp_reduction_helper_base : public T {
        /* This is only example code. The version used is the one which is
         * specialized for extra == 1.
         */
        static const int n = T::n;
        static const typename is_true<n>= _extra>::type extra = _extra;
...
template<int extra, typename T>
    struct gfp_reduction_helper: public gfp_reduction_helper_base<extra, T> {};

template<typename T>
    class gfp_reduction_helper<1, T>: public gfp_reduction_helper_base<1, T>
    {
        typedef gfp_reduction_helper_base<1, T> super;
        static const int n = T::n;
...
template<int _n, int _extra=1>
    class gfp : public gfp_reduction_helper<_extra, gfp_base<_n, _extra>>
{
    typedef gfp_reduction_helper<_extra, gfp_base<_n, _extra>> super;
    public:
    static const int n = _n;
    using typename super::elt;
    using typename super::elt_ur;
