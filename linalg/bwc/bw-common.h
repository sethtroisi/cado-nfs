 ...
 * this one ? */
extern char matrix_filename[FILENAME_MAX];

/* Whether the current job/thread may print to stdout */
extern int can_print;
 ...
 */
extern int mpi_split[2];

/* Similar, but for threads. Not exclusive of the above.
 */
extern int thr_split[2];

/* Only prep and lingen are not deterministic. */
extern int seed;

/* This is exposed only for hacks */
extern param_list pl;

#ifdef __cplusplus
extern "C" {
#endif

extern int bw_common_init(int argc, char * argv[]);
extern int bw_common_init_mpi(int argc, char * argv[]);
extern int bw_common_clear();
