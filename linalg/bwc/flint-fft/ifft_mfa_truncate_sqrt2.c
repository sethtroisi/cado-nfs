			    ...
			    mp_bitcnt_t b1, mp_bitcnt_t b2)
{
    ...
    if (negate1)
	mpn_neg_n(s, s, limbs + 1);
    mpn_div_2expmod_2expp1(s, s, limbs, b1);
    if (negate2)
	mpn_neg_n(t, t, limbs + 1);
			 ...
			 mp_size_t c, mp_size_t rs)
{
    ...
    ifft_radix2_twiddle(ii + n * is, is, n / 2, 2 * w, t1, t2, ws, r + rs, c,
			2 * rs);
			    ...
			    mp_size_t c, mp_size_t rs, mp_size_t trunc)
{
	...
	ifft_truncate1_twiddle(ii, is, n / 2, 2 * w, t1, t2, ws, r, c, 2 * rs,
			       trunc);

	for (i = 0; i < trunc; i++) {
#if HAVE_ADDSUB_N
	    mpn_addsub_n(ii[i * is], ii[i * is], ii[i * is], ii[(n + i) * is],
			 limbs + 1);
#else
	...
	for (i = trunc - n; i < n; i++) {
	    mpn_sub_n(ii[(i + n) * is], ii[i * is], ii[(i + n) * is],
		      limbs + 1);
	...
	ifft_truncate1_twiddle(ii + n * is, is, n / 2, 2 * w, t1, t2, ws,
			       r + rs, c, 2 * rs, trunc - n);

	for (i = 0; i < trunc - n; i++) {
	    ifft_butterfly(*t1, *t2, ii[i * is], ii[(n + i) * is], i, limbs,
			   w);
			     ...
			     mp_limb_t ** temp, mp_size_t n1, mp_size_t trunc)
{
	...
	/* 
	 * IFFT of length n2 on column i, applying z^{r*i} for rows going up
	...
	for (; j < n2; j++) {
	    ...
	    if (w & 1) {
		if (i & 1)
		    fft_adjust_sqrt2(ii[i + j * n1], ii[u - 2 * n], u, limbs,
				     w, *temp);
		else
		    fft_adjust(ii[i + j * n1], ii[u - 2 * n], u / 2, limbs,
			       w);
	    } else
	 ...
	 * IFFT of length n2 on column i, applying z^{r*i} for rows going up
	 * in steps of 1 starting at row 0, where z => w bits */
	ifft_truncate1_twiddle(ii + i, n1, n2 / 2, w * n1, t1, t2, w, 0, i, 1,
			       trunc2);
	...
	if (w & 1) {
		...
		if (j & 1)
		    ifft_butterfly_sqrt2(*t1, *t2, ii[j - 2 * n], ii[j], j,
					 limbs, w, *temp);
		else
		    ifft_butterfly(*t1, *t2, ii[j - 2 * n], ii[j], j / 2,
				   limbs, w);
	    ...
	    for (j = i; j < trunc - 2 * n; j += n1) {
		ifft_butterfly(*t1, *t2, ii[j - 2 * n], ii[j], j, limbs,
			       w / 2);
...
#pragma omp parallel for private(i, j, k)
	 ...
	 * IFFT of length n2 on column i, applying z^{r*i} for rows going up
	 * in steps of 1 starting at row 0, where z => w bits */
	ifft_radix2_twiddle(ii + i, n1, n2 / 2, w * n1, t1 + k, t2 + k, w, 0,
			    i, 1);
    }
...
#pragma omp parallel for private(i, j, k)
	 ...
	 * IFFT of length n2 on column i, applying z^{r*i} for rows going up
	 * in steps of 1 starting at row 0, where z => w bits */
	ifft_truncate1_twiddle(ii + i, n1, n2 / 2, w * n1, t1 + k, t2 + k, w,
			       0, i, 1, trunc2);
	...
	if (w & 1) {
	    for (j = i; j < trunc - 2 * n; j += n1) {
		if (j & 1)
		    ifft_butterfly_sqrt2(t1[k], t2[k], ii[j - 2 * n], ii[j],
					 j, limbs, w, temp[k]);
		else
		    ifft_butterfly(t1[k], t2[k], ii[j - 2 * n], ii[j], j / 2,
				   limbs, w);
	    ...
	    for (j = i; j < trunc - 2 * n; j += n1) {
		ifft_butterfly(t1[k], t2[k], ii[j - 2 * n], ii[j], j, limbs,
			       w / 2);
