...
#endif
/* 
 * Copyright (C) 2011 Fredrik Johansson Copyright (C) 2016 Claus Fieker
 * Copyright (C) 2016 William Hart. Copyright (C) 2018 Daniel Schultz
 * 
...
{
    flint_printf
	("Exception (FLINT memory_manager). Unable to allocate memory (%ld).\n",
	 size);
    flint_abort();
}
				     ...
				     cleanup_function)
{
    ...
    flint_cleanup_functions = flint_realloc(flint_cleanup_functions,
					    (flint_num_cleanup_functions +
					     1) *
					    sizeof(flint_cleanup_function_t));
...
void flint_cleanup_master()
{
     ...
     * thread_pool_clear(global_thread_pool); global_thread_pool_initialized
     * = 0; } */
    flint_cleanup();
}
