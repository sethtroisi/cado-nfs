...
static __inline__ mp_limb_t n_primes_next(n_primes_t iter)
{
...
static __inline__ double n_precompute_inverse(mp_limb_t n)
{
    return (double) 1 / (double) n;
}

static __inline__ mp_limb_t n_preinvert_limb(mp_limb_t n)
{
    mp_limb_t norm, ninv;
...
static __inline__ mp_limb_t n_addmod(mp_limb_t x, mp_limb_t y, mp_limb_t n)
{
    return (n - y > x ? x + y : x + y - n);
}

static __inline__ mp_limb_t n_submod(mp_limb_t x, mp_limb_t y, mp_limb_t n)
{
    return (y > x ? x - y + n : x - y);
}

static __inline__ mp_limb_t n_negmod(mp_limb_t x, mp_limb_t n)
{
...
static __inline__ mp_limb_t n_gcd_full(mp_limb_t x, mp_limb_t y)
{
    if (x >= y)
	return n_gcd(x, y);
    else
...
static __inline__ void n_factor_init(n_factor_t * factors)
{
    factors->num = UWORD(0);
}
...
mp_limb_t n_factor_trial_partial(n_factor_t * factors, mp_limb_t n,
				 mp_limb_t * prod, ulong num_primes,
				 mp_limb_t limit);
