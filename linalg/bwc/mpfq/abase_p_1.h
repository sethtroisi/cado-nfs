...
#define HAVE_abase_p_1_poly_random2
static inline
void abase_p_1_poly_random2(abase_p_1_dst_field, abase_p_1_dst_poly, unsigned int);
static inline
...
int abase_p_1_poly_deg(abase_p_1_dst_field K MAYBE_UNUSED, abase_p_1_src_poly w)
{
    if (w->size == 0)
        return -1;
    int deg = w->size-1;
    while ((deg >= 0) && (abase_p_1_cmp_ui(K, (w->c)[deg], 0) == 0))
        deg--;
    return deg;
}
...
void abase_p_1_poly_add(abase_p_1_dst_field k MAYBE_UNUSED, abase_p_1_dst_poly w, abase_p_1_src_poly u, abase_p_1_src_poly v)
{
    ...
    if (u->size <= v->size) {
        ...
        abase_p_1_vec_set(k, (w->c)+(u->size), (v->c)+(u->size), v->size-u->size);
    } else {
        abase_p_1_vec_add(k, w->c, u->c, v->c, v->size);
        abase_p_1_vec_set(k, (w->c)+(v->size), (u->c)+(v->size), u->size-v->size);
    }
    w->size=maxsize;
...
void abase_p_1_poly_add_ui(abase_p_1_dst_field k MAYBE_UNUSED, abase_p_1_dst_poly w, abase_p_1_src_poly u, unsigned long x)
{
    ...
    abase_p_1_add_ui(k, (w->c)[0], (u->c)[0], x);
    abase_p_1_vec_set(k, (w->c)+1, (u->c)+1, u->size - 1);
    w->size=u->size;
    unsigned int wdeg = abase_p_1_poly_deg(k, w);
    w->size=wdeg+1;
}
...
void abase_p_1_poly_sub_ui(abase_p_1_dst_field k MAYBE_UNUSED, abase_p_1_dst_poly w, abase_p_1_src_poly u, unsigned long x)
{
    ...
    abase_p_1_sub_ui(k, (w->c)[0], (u->c)[0], x);
    abase_p_1_vec_set(k, (w->c)+1, (u->c)+1, u->size - 1);
    w->size=u->size;
    unsigned int wdeg = abase_p_1_poly_deg(k, w);
    w->size=wdeg+1;
...
static inline
void abase_p_1_poly_random2(abase_p_1_dst_field k MAYBE_UNUSED, abase_p_1_dst_poly w, unsigned int n)
{
    if (w->alloc < n) {
        abase_p_1_vec_reinit(k, &(w->c), w->alloc, n);
        w->alloc = n;
    }
    abase_p_1_vec_random2(k, w->c, n);
    w->size=n;
    int wdeg = abase_p_1_poly_deg(k, w);
    w->size=wdeg+1;
}
