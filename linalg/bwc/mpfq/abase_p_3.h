...
#define HAVE_abase_p_3_poly_random2
static inline
void abase_p_3_poly_random2(abase_p_3_dst_field, abase_p_3_dst_poly, unsigned int);
static inline
...
static inline
void abase_p_3_poly_random2(abase_p_3_dst_field k MAYBE_UNUSED, abase_p_3_dst_poly w, unsigned int n)
{
    if (w->alloc < n) {
        abase_p_3_vec_reinit(k, &(w->c), w->alloc, n);
        w->alloc = n;
    }
    abase_p_3_vec_random2(k, w->c, n);
    w->size=n;
    int wdeg = abase_p_3_poly_deg(k, w);
    w->size=wdeg+1;
}
