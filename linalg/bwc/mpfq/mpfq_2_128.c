...
static void mpfq_2_128_poly_preinv(mpfq_2_128_dst_field, mpfq_2_128_dst_poly, mpfq_2_128_src_poly, int);
static /* *Mpfq::defaults::poly::code_for_poly_divmod */
void mpfq_2_128_poly_preinv(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_dst_poly q, mpfq_2_128_src_poly p, int n)
{
...
void mpfq_2_128_poly_gcd(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_dst_poly d, mpfq_2_128_src_poly a, mpfq_2_128_src_poly b)
{
    // GCD with 0 copy other operand
    long dega = mpfq_2_128_poly_deg(K, a);
    if (dega == -1) {
        mpfq_2_128_poly_setmonic(K, d, b);
        return;
    }
    long degb = mpfq_2_128_poly_deg(K, b);
    if (degb == -1) {
        mpfq_2_128_poly_setmonic(K, d, a);
        return;
    }
    // GCD with constant poly yields 1.
    if ((degb == 0) || (dega == 0)) {
        mpfq_2_128_poly_setcoef_ui(K, d, 1, 0);
        d->size=1;
        return;
    }
    
    mpfq_2_128_poly aa, bb;
    mpfq_2_128_poly_init(K, aa, dega+1);
    mpfq_2_128_poly_init(K, bb, degb+1);
    mpfq_2_128_vec_set(K, aa->c, a->c, dega+1);
    mpfq_2_128_vec_set(K, bb->c, b->c, degb+1);
    aa->size=dega+1;
    bb->size=degb+1;
    
    // Force deg(A) < deg(B)
    mpfq_2_128_poly_struct * A, * B;
    if (dega < degb) {
        A = &(aa[0]);
        B = &(bb[0]);
    } else if (dega > degb) {
        ...
        mpfq_2_128_poly_divmod(K, NULL, aa, aa, bb);
        A = &(aa[0]);
        B = &(bb[0]);
    ...
    while (mpfq_2_128_poly_deg(K, A) > 0) {
        mpfq_2_128_poly_divmod(K, NULL, B, B, A);
        mpfq_2_128_poly_struct *tmp;
        tmp = A; A = B; B = tmp;
    }
    // now we have deg(B) > 0 and deg(A) <= 0
    
    if (mpfq_2_128_poly_deg(K, A) == -1) {
        mpfq_2_128_poly_setmonic(K, d, B);
    } else {
        assert (mpfq_2_128_poly_deg(K, A) == 0);
        mpfq_2_128_poly_setcoef_ui(K, d, 1, 0);
        d->size=1;
    }
    mpfq_2_128_poly_clear(K, aa);
    mpfq_2_128_poly_clear(K, bb);
}

static void mpfq_2_128_poly_swap(mpfq_2_128_dst_field, mpfq_2_128_dst_poly, mpfq_2_128_dst_poly);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_poly_swap(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_dst_poly U, mpfq_2_128_dst_poly V)
{
    mpfq_2_128_vec tmp;
...
static void mpfq_2_128_poly_IterHalfGCD(mpfq_2_128_dst_field, mpfq_2_128_polymat *, mpfq_2_128_dst_poly, mpfq_2_128_dst_poly, long);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_poly_IterHalfGCD(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_polymat * M, mpfq_2_128_dst_poly UU, mpfq_2_128_dst_poly VV, long d_red)
{
    mpfq_2_128_poly_setcoef_ui(K, M->c00, 1, 0);
    M->c00->size=1;
    mpfq_2_128_poly_setcoef_ui(K, M->c11, 1, 0);
    M->c11->size=1;
    M->c01->size=0;
    M->c10->size=0;
    
    long goal = mpfq_2_128_poly_deg(K, UU) - d_red;
    
    if (mpfq_2_128_poly_deg(K, VV) <= goal)
        return;
    
    mpfq_2_128_poly_struct *U, *V;
    U = &UU[0]; V = &VV[0];
    
    mpfq_2_128_poly Q, t;
    mpfq_2_128_poly_init(K, Q, 0);
    mpfq_2_128_poly_init(K, t, 0);
    while (mpfq_2_128_poly_deg(K, V) > goal) {
        mpfq_2_128_poly_divmod(K, Q, U, U, V);
        {
            mpfq_2_128_poly_struct * tmp;
            tmp = U; U = V; V = tmp;
        }
    
        mpfq_2_128_poly_mul(K, t, Q, M->c10);
        mpfq_2_128_poly_sub(K, t, M->c00, t);
        mpfq_2_128_poly_set(K, M->c00, M->c10);
        mpfq_2_128_poly_set(K, M->c10, t);
    
        mpfq_2_128_poly_mul(K, t, Q, M->c11);
        mpfq_2_128_poly_sub(K, t, M->c01, t);
        mpfq_2_128_poly_set(K, M->c01, M->c11);
        mpfq_2_128_poly_set(K, M->c11, t);
    }
    if (U != &UU[0]) {
        mpfq_2_128_poly_swap(K, UU, VV);
    }
    mpfq_2_128_poly_clear(K, Q);
    mpfq_2_128_poly_clear(K, t);
}

static void mpfq_2_128_poly_mat_mul(mpfq_2_128_dst_field, mpfq_2_128_polymat *, mpfq_2_128_dst_poly, mpfq_2_128_dst_poly);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_poly_mat_mul(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_polymat * M, mpfq_2_128_dst_poly U, mpfq_2_128_dst_poly V)
{
    // (U, V)^t := M * (U, V)^t
    mpfq_2_128_poly aux1, aux2, aux3;
    mpfq_2_128_poly_init(K, aux1, 0);
    mpfq_2_128_poly_init(K, aux2, 0);
    mpfq_2_128_poly_init(K, aux3, 0);
    mpfq_2_128_poly_mul(K, aux1, M->c00, U);
    mpfq_2_128_poly_mul(K, aux2, M->c01, V);
    mpfq_2_128_poly_add(K, aux2, aux2, aux1);
    
    mpfq_2_128_poly_mul(K, aux1, M->c10, U);
    mpfq_2_128_poly_mul(K, aux3, M->c11, V);
    mpfq_2_128_poly_add(K, V, aux1, aux3);
    mpfq_2_128_poly_set(K, U, aux2);
    mpfq_2_128_poly_clear(K, aux1);
    mpfq_2_128_poly_clear(K, aux2);
    mpfq_2_128_poly_clear(K, aux3);
}

static void mpfq_2_128_polymat_mul(mpfq_2_128_dst_field, mpfq_2_128_polymat *, mpfq_2_128_polymat *, mpfq_2_128_polymat *);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_polymat_mul(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_polymat * M, mpfq_2_128_polymat * M1, mpfq_2_128_polymat * M2)
{
    // M = M1*M2
    // TODO: Strassen!!!
    mpfq_2_128_poly aux1, aux2;
    mpfq_2_128_poly_init(K, aux1, 0);
    mpfq_2_128_poly_init(K, aux2, 0);
    mpfq_2_128_poly_mul(K, aux1, M1->c00, M2->c00);
    mpfq_2_128_poly_mul(K, aux2, M1->c01, M2->c10);
    mpfq_2_128_poly_add(K, M->c00, aux2, aux1);
    mpfq_2_128_poly_mul(K, aux1, M1->c00, M2->c01);
    mpfq_2_128_poly_mul(K, aux2, M1->c01, M2->c11);
    mpfq_2_128_poly_add(K, M->c01, aux2, aux1);
    mpfq_2_128_poly_mul(K, aux1, M1->c10, M2->c00);
    mpfq_2_128_poly_mul(K, aux2, M1->c11, M2->c10);
    mpfq_2_128_poly_add(K, M->c10, aux2, aux1);
    mpfq_2_128_poly_mul(K, aux1, M1->c10, M2->c01);
    mpfq_2_128_poly_mul(K, aux2, M1->c11, M2->c11);
    mpfq_2_128_poly_add(K, M->c11, aux2, aux1);
    mpfq_2_128_poly_clear(K, aux1);
    mpfq_2_128_poly_clear(K, aux2);
}

static void mpfq_2_128_poly_HalfGCD(mpfq_2_128_dst_field, mpfq_2_128_polymat *, mpfq_2_128_dst_poly, mpfq_2_128_dst_poly, long);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_poly_HalfGCD(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_polymat * M, mpfq_2_128_dst_poly U, mpfq_2_128_dst_poly V, long d_red)
{
    if ((mpfq_2_128_poly_deg(K, V) == -1) ||
            (mpfq_2_128_poly_deg(K, V) <= (mpfq_2_128_poly_deg(K, U) - d_red))) {
        mpfq_2_128_poly_setcoef_ui(K, M->c00, 1, 0);
        M->c00->size=1;
        mpfq_2_128_poly_setcoef_ui(K, M->c11, 1, 0);
        M->c11->size=1;
    ...
    long n = mpfq_2_128_poly_deg(K, U) - 2*d_red + 2;
    if (n < 0) n = 0;
    
    mpfq_2_128_poly U1, V1;
    mpfq_2_128_poly_init(K, U1, mpfq_2_128_poly_deg(K, U)-n+1);
    mpfq_2_128_poly_init(K, V1, mpfq_2_128_poly_deg(K, V)-n+1);
    
    mpfq_2_128_vec_set(K, U1->c, U->c+n, MAX(mpfq_2_128_poly_deg(K, U)+1-n, 0));
    mpfq_2_128_vec_set(K, V1->c, V->c+n, MAX(mpfq_2_128_poly_deg(K, V)+1-n, 0));
    U1->size = MAX(mpfq_2_128_poly_deg(K, U)-n+1, 0);
    V1->size = MAX(mpfq_2_128_poly_deg(K, V)-n+1, 0);
#define GCD_THRESHOLD 50
    if (d_red <= GCD_THRESHOLD) {
        mpfq_2_128_poly_IterHalfGCD(K, M, U1, V1, d_red);
        mpfq_2_128_poly_clear(K, U1);
        mpfq_2_128_poly_clear(K, V1);
        return;
    }
    ...
    mpfq_2_128_polymat M1;
    mpfq_2_128_poly_init(K, M1.c00, 0); mpfq_2_128_poly_init(K, M1.c01, 0);
    mpfq_2_128_poly_init(K, M1.c10, 0); mpfq_2_128_poly_init(K, M1.c11, 0);
    
    mpfq_2_128_poly_HalfGCD(K, &M1, U1, V1, d1);
    mpfq_2_128_poly_mat_mul(K, &M1, U1, V1);
    
    long d2 = mpfq_2_128_poly_deg(K, V1) - mpfq_2_128_poly_deg(K, U) + n + d_red;
    
    if ((mpfq_2_128_poly_deg(K, V1) == -1) || d2 <= 0) {
        mpfq_2_128_poly_set(K, M->c00, M1.c00);
        mpfq_2_128_poly_set(K, M->c01, M1.c01);
        mpfq_2_128_poly_set(K, M->c10, M1.c10);
        mpfq_2_128_poly_set(K, M->c11, M1.c11);
        mpfq_2_128_poly_clear(K, M1.c00); mpfq_2_128_poly_clear(K, M1.c01);
        mpfq_2_128_poly_clear(K, M1.c10); mpfq_2_128_poly_clear(K, M1.c11);
        return;
    }
    
    mpfq_2_128_poly Q;
    mpfq_2_128_polymat M2;
    mpfq_2_128_poly_init(K, Q, 0);
    mpfq_2_128_poly_init(K, M2.c00, 0); mpfq_2_128_poly_init(K, M2.c01, 0);
    mpfq_2_128_poly_init(K, M2.c10, 0); mpfq_2_128_poly_init(K, M2.c11, 0);
    
    mpfq_2_128_poly_divmod(K, Q, U1, U1, V1);
    mpfq_2_128_poly_swap(K, U1, V1);
    
    mpfq_2_128_poly_HalfGCD(K, &M2, U1, V1, d2);
    
    mpfq_2_128_poly t;
    mpfq_2_128_poly_init(K, t, 0);
    
    mpfq_2_128_poly_mul(K, t, Q, M1.c10);
    mpfq_2_128_poly_sub(K, t, M1.c00, t);
    mpfq_2_128_poly_swap(K, M1.c00, M1.c10);
    mpfq_2_128_poly_swap(K, M1.c10, t);
    
    mpfq_2_128_poly_mul(K, t, Q, M1.c11);
    mpfq_2_128_poly_sub(K, t, M1.c01, t);
    mpfq_2_128_poly_swap(K, M1.c01, M1.c11);
    mpfq_2_128_poly_swap(K, M1.c11, t);
    
    mpfq_2_128_poly_clear(K, t);
    mpfq_2_128_poly_clear(K, Q);
    
    mpfq_2_128_polymat_mul(K, M, &M2, &M1);
    mpfq_2_128_poly_clear(K, M1.c00); mpfq_2_128_poly_clear(K, M1.c01);
    mpfq_2_128_poly_clear(K, M1.c10); mpfq_2_128_poly_clear(K, M1.c11);
    mpfq_2_128_poly_clear(K, M2.c00); mpfq_2_128_poly_clear(K, M2.c01);
    mpfq_2_128_poly_clear(K, M2.c10); mpfq_2_128_poly_clear(K, M2.c11);
}

static void mpfq_2_128_poly_xHalfGCD(mpfq_2_128_dst_field, mpfq_2_128_polymat *, mpfq_2_128_dst_poly, mpfq_2_128_dst_poly, long);
static /* *Mpfq::defaults::poly::code_for_poly_xgcd */
void mpfq_2_128_poly_xHalfGCD(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_polymat * M, mpfq_2_128_dst_poly U, mpfq_2_128_dst_poly V, long d_red)
{
    if ((mpfq_2_128_poly_deg(K, V) == -1) ||
            (mpfq_2_128_poly_deg(K, V) <= (mpfq_2_128_poly_deg(K, U) - d_red))) {
        mpfq_2_128_poly_setcoef_ui(K, M->c00, 1, 0);
    ...
    long du = mpfq_2_128_poly_deg(K, U);
#define XGCD_THRESHOLD 50
    if (d_red <= XGCD_THRESHOLD) {
        mpfq_2_128_poly_IterHalfGCD(K, M, U, V, d_red);
        return;
    }
    ...
    mpfq_2_128_poly_HalfGCD(K, &M1, U, V, d1);
    mpfq_2_128_poly_mat_mul(K, &M1, U, V);
    
    long d2 = mpfq_2_128_poly_deg(K, V) - du + d_red;
    
    if ((mpfq_2_128_poly_deg(K, V) == -1) || d2 <= 0) {
        mpfq_2_128_poly_set(K, M->c00, M1.c00);
        mpfq_2_128_poly_set(K, M->c01, M1.c01);
        mpfq_2_128_poly_set(K, M->c10, M1.c10);
        mpfq_2_128_poly_set(K, M->c11, M1.c11);
    ...
    mpfq_2_128_poly_divmod(K, Q, U, U, V);
    mpfq_2_128_poly_swap(K, U, V);
    
    mpfq_2_128_poly_xHalfGCD(K, &M2, U, V, d2);
...
void mpfq_2_128_poly_xgcd(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_dst_poly d, mpfq_2_128_dst_poly s, mpfq_2_128_dst_poly t, mpfq_2_128_src_poly a, mpfq_2_128_src_poly b)
{
    if ((mpfq_2_128_poly_deg(K, a) == -1) && (mpfq_2_128_poly_deg(K, b) == -1)) {
        d->size=0;
        t->size=0;
        mpfq_2_128_poly_setcoef_ui(K, s, 1, 0);
        s->size=1;
        return;
    }
    
    mpfq_2_128_poly UU, VV, Q;
    mpfq_2_128_poly_init(K, UU, 0);
    mpfq_2_128_poly_init(K, VV, 0);
    mpfq_2_128_poly_init(K, Q, 0);
    mpfq_2_128_poly_struct *U, *V;
    U = &UU[0]; V = &VV[0];
    
    mpfq_2_128_poly_set(K, U, a);
    mpfq_2_128_poly_set(K, V, b);
    
    long flag = 0;
    
    if (mpfq_2_128_poly_deg(K, U) == mpfq_2_128_poly_deg(K, V)) {
        mpfq_2_128_poly_divmod(K, Q, U, U, V);
        mpfq_2_128_poly_struct *tmp;
        tmp = U; U = V; V = tmp;
        flag = 1;
    } else if (mpfq_2_128_poly_deg(K, U) < mpfq_2_128_poly_deg(K, V)) {
        mpfq_2_128_poly_struct *tmp;
        tmp = U; U = V; V = tmp;
        flag = 2;
    }
    
    mpfq_2_128_polymat M;
    mpfq_2_128_poly_init(K, M.c00, 0);
    mpfq_2_128_poly_init(K, M.c01, 0);
    mpfq_2_128_poly_init(K, M.c10, 0);
    mpfq_2_128_poly_init(K, M.c11, 0);
    
    mpfq_2_128_poly_xHalfGCD(K, &M, U, V, mpfq_2_128_poly_deg(K, U)+1);
    
    mpfq_2_128_poly_set(K, d, U);
    
    if (flag == 0) {
        mpfq_2_128_poly_set(K, s, M.c00);
        mpfq_2_128_poly_set(K, t, M.c01);
    } else if (flag == 1) {
        mpfq_2_128_poly_set(K, s, M.c01);
        mpfq_2_128_poly_mul(K, t, Q, M.c01);
        mpfq_2_128_poly_sub(K, t, M.c00, t);
    } else {
        mpfq_2_128_poly_set(K, s, M.c01);
        mpfq_2_128_poly_set(K, t, M.c00);
    }
    
    mpfq_2_128_elt w;
    mpfq_2_128_init(K, &w);
    mpfq_2_128_poly_getcoef(K, w, d, mpfq_2_128_poly_deg(K, d));
    mpfq_2_128_inv(K, w, w);
    mpfq_2_128_poly_scal_mul(K, d, d, w);
    mpfq_2_128_poly_scal_mul(K, s, s, w);
    mpfq_2_128_poly_scal_mul(K, t, t, w);
    mpfq_2_128_clear(K, &w);
    
    mpfq_2_128_poly_clear(K, M.c00);
    mpfq_2_128_poly_clear(K, M.c01);
    mpfq_2_128_poly_clear(K, M.c10);
    mpfq_2_128_poly_clear(K, M.c11);
    
    mpfq_2_128_poly_clear(K, UU);
    mpfq_2_128_poly_clear(K, VV);
    mpfq_2_128_poly_clear(K, Q);
}
