...
static inline
void mpfq_2_128_vec_set_partial(mpfq_2_128_dst_field, mpfq_2_128_dst_vec, mpfq_2_128_src_vec, unsigned int, unsigned int, unsigned int);
static inline
...
static inline
void mpfq_2_128_vec_set_partial(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_vec w, mpfq_2_128_src_vec u, unsigned int bw, unsigned int bu, unsigned int l)
{
    unsigned int i;
    for(i = 0; i < l; ++i)
        mpfq_2_128_set(k, w[bw+i], u[bu+i]);
}
...
void mpfq_2_128_poly_setcoef(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_poly w, mpfq_2_128_src_elt x, unsigned int i)
{
    ...
    if (w->size < (i+1)) {
        for (j = w->size; j < i; ++j) {
            mpfq_2_128_vec_setcoef_ui(k, w->c, 0, j);
        }  
        w->size = i+1;
    }
}
...
int mpfq_2_128_poly_deg(mpfq_2_128_dst_field K MAYBE_UNUSED, mpfq_2_128_src_poly w)
{
    ...
    mpfq_2_128_vec_getcoef(K, temp, w->c, deg);
...
void mpfq_2_128_poly_add(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_poly w, mpfq_2_128_src_poly u, mpfq_2_128_src_poly v)
{
    ...
    if (u->size <= v->size) {
        ...
        mpfq_2_128_vec_set_partial(k, (w->c), (v->c), u->size, u->size, v->size-u->size);
    } else {
        mpfq_2_128_vec_add(k, w->c, u->c, v->c, v->size);
        mpfq_2_128_vec_set_partial(k, (w->c), (u->c), v->size, v->size, u->size-v->size);
    }
    w->size=maxsize;
...
void mpfq_2_128_poly_sub(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_poly w, mpfq_2_128_src_poly u, mpfq_2_128_src_poly v)
{
    ...
    if (u->size <= v->size) {
        ...
        for (i = u->size; i< v->size; ++i) {
            mpfq_2_128_poly_getcoef(k, temp, v, i);
            mpfq_2_128_neg(k, temp, temp);
            mpfq_2_128_poly_setcoef(k, w, temp, i);
        }
    } else {
    ...
    w->size=maxsize;
    //mpfq_2_128_poly_neg(k, w, v);
    //mpfq_2_128_poly_add(k, w, u, w);
    unsigned int wdeg = mpfq_2_128_poly_deg(k, w);
    w->size=wdeg+1;
}
...
void mpfq_2_128_poly_add_ui(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_poly w, mpfq_2_128_src_poly u, unsigned long x)
{
    ...
    mpfq_2_128_elt temp;
    mpfq_2_128_init(k, &temp);
    mpfq_2_128_poly_getcoef(k, temp, u, 0);
    mpfq_2_128_add_ui(k, temp, temp, x);
    mpfq_2_128_vec_setcoef(k, w->c, temp, 0);
    mpfq_2_128_vec_set_partial(k, w->c, u->c, 1, 1, u->size-1);
    w->size=u->size;
    mpfq_2_128_clear(k, &temp);
}
...
void mpfq_2_128_poly_sub_ui(mpfq_2_128_dst_field k MAYBE_UNUSED, mpfq_2_128_dst_poly w, mpfq_2_128_src_poly u, unsigned long x)
{
    ...
    mpfq_2_128_init(k, &temp);
    mpfq_2_128_poly_getcoef(k, temp, u, 0);
    mpfq_2_128_sub_ui(k, temp, temp, x);
    mpfq_2_128_vec_setcoef(k, w->c, temp, 0);
    mpfq_2_128_vec_set_partial(k, w->c, u->c, 1, 1, u->size-1);
    w->size=u->size;
    mpfq_2_128_clear(k, &temp);
