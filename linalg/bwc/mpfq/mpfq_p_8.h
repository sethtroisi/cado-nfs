...
void mpfq_p_8_field_specify(mpfq_p_8_dst_field, unsigned long, const void *);
...
int mpfq_p_8_vec_hamming_weight(mpfq_p_8_dst_field, mpfq_p_8_src_vec, unsigned int);
int mpfq_p_8_vec_find_first_set(mpfq_p_8_dst_field, mpfq_p_8_src_vec, unsigned int);
int mpfq_p_8_vec_simd_hamming_weight(mpfq_p_8_dst_field, mpfq_p_8_src_vec, unsigned int);
int mpfq_p_8_vec_simd_find_first_set(mpfq_p_8_dst_field, mpfq_p_8_src_vec, unsigned int);
...
void mpfq_p_8_add_dotprod(mpfq_p_8_dst_field, mpfq_p_8_dst_vec, mpfq_p_8_src_vec, mpfq_p_8_src_vec, unsigned int);
...
{
    mp_limb_t cy;
    cy = mpfq_fixmp_8_add(z, x, y);
    if (cy || (mpfq_fixmp_8_cmp(z, k->p->_mp_d) >= 0))
        mpfq_fixmp_8_sub(z, z, k->p->_mp_d);
}
...
{
    mp_limb_t cy;
    cy = mpfq_fixmp_8_sub(z, x, y);
    if (cy) // negative result
        mpfq_fixmp_8_add(z, z, k->p->_mp_d);
}
...
{
    if (mpfq_fixmp_8_cmp_ui(x, 0))
        mpfq_fixmp_8_sub(z, k->p->_mp_d, x);
    else {
        int i;
        for (i = 0; i < 8; ++i)
            z[i] = 0;
...
{
    mp_limb_t tmp[16];
    mpfq_fixmp_8_mul(tmp, x, y);
    mpfq_fixmp_8_mod(z, tmp, k->p->_mp_d);
}
...
{
    if (mpfq_fixmp_8_cmp(x,k->p->_mp_d)>=0) {
      mp_limb_t q[8+1];
      mpfq_p_8_elt r;
...
{
    int ret=mpfq_fixmp_8_invmod(z, x, k->p->_mp_d);
    if (!ret)
...
{
    mpfq_copy(r, s, 8); mpfq_zero(r+8, 17-8);
}
...
{
    return mpfq_fixmp_8_cmp_ui(x,y);
}
...
{
            mpfq_p_8_dst_elt y = mpfq_p_8_vec_coeff_ptr(K, w, i);
            mpfq_p_8_set(K, y, x);
}
...
{
            mpfq_p_8_src_elt y = mpfq_p_8_vec_coeff_ptr_const(K, w, i);
            mpfq_p_8_set(K, x, y);
}
...
static inline
void mpfq_p_8_vec_scal_mul(mpfq_p_8_dst_field K MAYBE_UNUSED, mpfq_p_8_dst_vec w, mpfq_p_8_src_vec u, mpfq_p_8_src_elt c, unsigned int n)
{
        unsigned int i;
    for(i = 0; i < n; i++) {
        mpfq_p_8_src_elt x = mpfq_p_8_vec_coeff_ptr_const(K, u, i);
        mpfq_p_8_dst_elt y = mpfq_p_8_vec_coeff_ptr(K, w, i);
        mpfq_p_8_mul(K, y, x, c);
    }
}
...
void mpfq_p_8_vec_ur_set_vec(mpfq_p_8_dst_field K MAYBE_UNUSED, mpfq_p_8_dst_vec_ur w, mpfq_p_8_src_vec u, unsigned int n)
{
            ...
            for(i = 0; i < n; ++i) {
                mpfq_p_8_src_elt x = mpfq_p_8_vec_coeff_ptr_const(K, u, i);
                mpfq_p_8_dst_elt_ur y = mpfq_p_8_vec_ur_coeff_ptr(K, w, i);
                mpfq_p_8_elt_ur_set_elt(K, y, x);
            }
}
...
{
            mpfq_p_8_src_elt_ur y = mpfq_p_8_vec_ur_coeff_ptr_const(K, w, i);
            mpfq_p_8_elt_ur_set(K, x, y);
}
...
static inline
void mpfq_p_8_vec_scal_mul_ur(mpfq_p_8_dst_field K MAYBE_UNUSED, mpfq_p_8_dst_vec_ur w, mpfq_p_8_src_vec u, mpfq_p_8_src_elt c, unsigned int n)
{
    unsigned int i;
    for(i = 0; i < n; i++) {
        mpfq_p_8_src_elt x = mpfq_p_8_vec_coeff_ptr_const(K, u, i);
        mpfq_p_8_dst_elt_ur y = mpfq_p_8_vec_ur_coeff_ptr(K, w, i);
        mpfq_p_8_mul_ur(K, y, x, c);
    }
}
...
void mpfq_p_8_vec_reduce(mpfq_p_8_dst_field K MAYBE_UNUSED, mpfq_p_8_dst_vec w, mpfq_p_8_dst_vec_ur u, unsigned int n)
{
    unsigned int i;
    for(i = 0; i < n; i++) {
        mpfq_p_8_dst_elt_ur x = mpfq_p_8_vec_ur_coeff_ptr(K, u, i);
        mpfq_p_8_dst_elt y = mpfq_p_8_vec_coeff_ptr(K, w, i);
        mpfq_p_8_reduce(K, y, x);
    }
}
...
void mpfq_p_8_poly_xgcd(mpfq_p_8_dst_field k MAYBE_UNUSED, mpfq_p_8_dst_poly g, mpfq_p_8_dst_poly u0, mpfq_p_8_dst_poly v0, mpfq_p_8_src_poly a0, mpfq_p_8_src_poly b0)
{
    ...
    mpfq_p_8_elt c;
    mpfq_p_8_init(k,&c);
    mpfq_p_8_set_ui(k,c,0);        /* placate gcc */
