...
void adjust_distribution_parameters(distribution_data_ptr f, unsigned long nrows, unsigned long ncols, double density)
{
    ...
    if (verbose) {
        fprintf(stderr, "Expected row weight: %.3f, sdev %.3f\nrows", f->mean, f->sdev);
        fprintf(stderr, "Expected weight for first column is %.3f (sdev %.3f, m/sdev=%.1f)\n",
                mean0, sdev0, mean0 / sdev0);
        fprintf(stderr, "Expected weight for last column is %.3f (sdev %.3f, m/sdev=%.1f)\n",
                mean_n, sdev_n, mean_n / sdev_n);
        fprintf(stderr, "Tail size for rejected normal picks is 2^%.2f\n",
                log2(1-erf(mean_n / sdev_n)));
        fprintf(stderr, "Worst-case expectation for last column weight by normal approximation: %.3f\n",
