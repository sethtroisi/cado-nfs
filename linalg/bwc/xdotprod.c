...
void x_dotprod(matmul_top_data_ptr mmt, uint32_t * xv, unsigned int nx, mpfq_vbase_ptr A, void * v, unsigned int z0, unsigned int m, int sign)
{
    ...
    for(unsigned int j = 0 ; j < m ; j++) {
        void * where = A->vec_subvec(A, v, z0 + j);
