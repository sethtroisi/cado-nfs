...
struct sse2_2words_traits {
...
#endif
    static const unsigned int sse2_vectoring = 2;
    ...
    static int can() {
        return globals::nbys == sse2_vectoring && MODBITS() < 20;
    }
...
/*
   ...
   sse2_2words_traits::scalar_t const& x)
   {
   sse2_2words_traits::vec_t foo;
   memcpy(foo, &x.p, sizeof(foo));
   for(int i = 0 ; i < sse2_2words_traits::sse2_vectoring ; i++) {
   if (i) o << " ";
   o << foo[i];
   }
   return o;
