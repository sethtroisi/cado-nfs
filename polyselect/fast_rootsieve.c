...
void rootsieve(poly_t f,poly_t g, long U, long V, unsigned long prime_bound ) {  // Ready
    ...
    for ( l=0 ; l<p ; l++) {
      ...
      minus_f_over_gmodp_l = mpz_get_si(mp_tmp);     
			...
			if (m==0) {
					...
					minus_f_over_gmodp_l = (long)modular_product((unsigned long)(p-minus_f_over_gmodp_l),(unsigned long)igl,p);
			}
...
long light (double * alpha, long u0, long v0, long um, long vm, long us, long vs, long skew, double contribution) {
