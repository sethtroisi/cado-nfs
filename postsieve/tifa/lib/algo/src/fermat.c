...
#if FERMAT_DEBUG
  #include <stdio.h>
  #define FERMAT_GMP_PRINT(...) gmp_printf(__VA_ARGS__)
  #define FERMAT_PRINT(...)     printf(__VA_ARGS__)
#else
  #define FERMAT_GMP_PRINT(...) /* intentionally left empty */
  #define FERMAT_PRINT(...)     /* intentionally left empty */
#endif
...
//
#define MAX_Y_BOUND_MULTIPLIER 0.2
...
static ecode_t update_fermat_context(factoring_machine_t* const machine
    ...
    START_TIMER;
    //
    // For the time being, just give up the factorization. Otherwise, one could
    // try the "basic" variant which, according to Pollard, is faster... on a
    // 7 Mhz Psion handheld computer with 256K of memory...
    //
...
static ecode_t perform_fermat(factoring_machine_t* const machine) {
    ...
    unsigned long int b     = mpz_get_ui(sqrtn) + 1;
    unsigned long int sqrtb = sqrt(b);
    unsigned long int max_y = sqrtb * MAX_Y_BOUND_MULTIPLIER;

    mpz_init_set_ui(pz, sqrtb>>4);
    mpz_init(pz2);
    mpz_init(s);
    mpz_init(x0);
    mpz_init(xi);
...
//            if (0 == mpz_fits_ulong_p(ri)) {
//                gmp_printf("1. ri = %Zd\n", ri);
//            }
//            if (0 == mpz_fits_ulong_p(xi)) {
//                gmp_printf("1. xi = %Zd\n", xi);
//            }
//            if (0 == mpz_fits_ulong_p(yi)) {
//                gmp_printf("1. yi = %Zd\n", yi);
//            }
            //
