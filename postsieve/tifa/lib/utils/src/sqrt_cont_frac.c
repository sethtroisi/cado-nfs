...
void init_cont_frac_state(cont_frac_state_t* const state, const mpz_t n) {
    ...
    // 2 * SIZ(A) - 1, we should make sure to allocate enough limbs for
    // state->_ztmp_...
    //
    ...
    //
    // state->_a0_ and state->_a1_ are bounded by n but the step functions use
    // mpz_swap's instead of mpz_set's, so _a0_ and _a1_ should have as many
    // limbs as state->a...
    //
    mpz_init2(state->_a0_, 2 * size_n);
    mpz_init2(state->_a1_, 2 * size_n);
    ...
    //
    mpz_set_ui(state->_a0_, 0);
    mpz_set_ui(state->_a1_, 1);
...
void step_function_mpz(cont_frac_state_t* const state, uint32_t nb_steps) {
    ...
    for (uint32_t i = 0; i < nb_steps; i++) {
        ...
        mpz_swap(state->_a1_, state->a);
        mpz_swap(state->a, state->_a0_);
        ...
        //mpz_swap(state->_b1_, state->b);
        //mpz_swap(state->b, state->_b0_);
...
void step_function_mpn(cont_frac_state_t* const state, uint32_t nb_steps) {
    ...
    for (uint32_t i = 0; i < nb_steps; i++) {
        ...
        mpz_addmul(state->a, state->t, state->_a1_);
#else
        MPN_MUL_CS(state->_ztmp_, state->t, state->_a1_);
