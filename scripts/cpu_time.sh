...
if [[ -z $1 || $(expr $1 : '.*[l].*') != 0 ]]
	...
	num_lines=$(grep -n Done. ${name}.bwc.stderr | sed "s/^\([0-9]*\):Done./\1-2/g")
	if [[ $(echo $num_lines | wc -w) == 2 ]]
		then echo -n "        krylov [real time]:   "
			 ...
			 v0=$(echo $line_v | sed "s/^.*WCT: \(\S*\) tot.*/\1/g" | f)
			 v1=$(echo $line_v | tail -1 | sed "s/^.*iter, \(\S*\) cpu.*/\1/g")
			 echo "$v0 ($v1)"
			 ...
			 s0=$(echo $line_s | sed "s/^.*WCT: \(\S*\) tot.*/\1/g" | f)
			 s1=$(echo $line_s | sed "s/^.*iter, \(\S*\) cpu.*/\1/g")
			 echo "$s0 ($s1)"
