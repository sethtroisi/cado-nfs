...
struct bucket_array_t {
  ...
  slice_index_t      max_slice_index; // maximum slice index in the factor
                                      // base, used for allocating memory
...
public:
  ...
  const bucket_update_t *cbegin(const size_t i_bucket, const slice_index_t i_slice) const {
    ASSERT_ALWAYS(i_slice < nr_slices);
  ...
  /* Return an end iterator over the bucket_update_t entries in i_bucket-th
     bucket, generated by the i_slice-th slice */
  const bucket_update_t *cend(const size_t i_bucket, const slice_index_t i_slice) const {
    ASSERT_ALWAYS(i_slice < nr_slices);
...
public:
  ...
  const UPDATE_TYPE * cbegin() const {return start;}
  const UPDATE_TYPE * cend() const {return write;}
...
static inline void
bucket_add_slice_index(bucket_array_t *BA, const slice_index_t slice_index)
{
  /* Write new set of pointers if the logp value changed */
  ASSERT_ALWAYS(BA->nr_slices < BA->max_slice_index);
  aligned_medium_memcpy((uint8_t *)BA->slice_start + BA->size_b_align * BA->nr_slices, BA->bucket_write, BA->size_b_align);
  BA->slice_index[BA->nr_slices++] = slice_index;
}
