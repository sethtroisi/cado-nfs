...
struct bucket_array_t : private NonCopyable {
  bucket_update_shorthint_t *big_data;
  size_t big_size;                    // size of bucket update memory

  bucket_update_shorthint_t ** bucket_write;    // Contains pointers to first empty
                                      // location in each bucket
  bucket_update_shorthint_t ** bucket_start;    // Contains pointers to beginning of
                                      // buckets
  bucket_update_shorthint_t ** bucket_read;     // Contains pointers to first unread
                                      // location in each bucket
  slice_index_t    * slice_index;     // For each slice that gets sieved,
                                      // new index is added here
  bucket_update_shorthint_t ** slice_start;     // For each slice there are
...
private:
  ...
  bucket_update_shorthint_t ** get_slice_pointers(const slice_index_t i_slice) const {
    ASSERT_ALWAYS(i_slice < nr_slices);
    ASSERT_ALWAYS(size_b_align % sizeof(bucket_update_shorthint_t *) == 0);
    return (slice_start + i_slice * size_b_align / sizeof(bucket_update_shorthint_t *));
  }
...
public:
  ...
  /* Return a begin iterator over the bucket_update_shorthint_t entries in i_bucket-th
     bucket, generated by the i_slice-th slice */
  const bucket_update_shorthint_t *begin(const size_t i_bucket, const slice_index_t i_slice) const {
    ASSERT_ALWAYS(i_slice < nr_slices);
    return get_slice_pointers(i_slice)[i_bucket];
  }

  void reset_pointers();
  /* Return an end iterator over the bucket_update_shorthint_t entries in i_bucket-th
     bucket, generated by the i_slice-th slice */
  const bucket_update_shorthint_t *end(const size_t i_bucket, const slice_index_t i_slice) const {
    ASSERT_ALWAYS(i_slice < nr_slices);
  ...
  void push_bucket_update(const int i, const bucket_update_shorthint_t update) {
