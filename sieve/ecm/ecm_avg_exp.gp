...
Chebyshev(i,X) = {
  ...
  if (i == 0, return (2)); 
  if (i == 1, return(X)); 
  if(i % 2 == 0, return (V (i/2, X)^2-2));
  if(i % 3 == 0, n = V (i/3, X); return (n^3 - 3 * n));
  if(i % 5 == 0, n = V (i/5, X); return (n^5 - 5 * n^3 + 5*n));
  return (V ((i+1)/2, X) * V ((i-1)/2, X) - X)
}
...
V_order (X) = {
  local(m, o, d, f, po, i, p);
  if (X == 0, return 4);
  m = lift(X) + lift(-X);
  o=m-kronecker (lift(X)^2-4, m);
  if (Chebyshev(o,X) != 2, error("V_order: group order should be ", o, " but isn't"));
  f = factorint (o)~;
  po = o;
  for (i=1, length(f), 
    p = f[1,i];
    while (po % p == 0 && Chebyshev(po/p, X) == 2, po /= p);
  );
  return (po);
}
...
list_pp1_order (pmin, pmax, res, mod, verbose) =
{
  ...
  while (p <= pmax,
    ...
    po = V_order (Mod(2/7, p));
    e = o / po;
    r *= e;
    s += e;
    if (verbose != 0, print(p, " ", o, " ", po, " ", e));
    /* print (e); */
    p = nextprime (p + 1);
  );
  print (n " primes, ", factorint (r));
}
