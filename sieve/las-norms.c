...
/* I have to manually "save" i in this function: if I work directly on i,
   gcc doesn't see i is modified. */
...
*/
static inline void uint64truncfastlog2(double io, double add, double scale, uint8_t *addr, ssize_t decal) {
#ifdef HAVE_SSE2
...
/* 2 values are computed + fabs at the beginning. These values are duplicated and
   written. CAREFUL! the input i is modified and its previous value is lost!
   */
...
/* 2 values are computed + fabs at the beginning. These values are written at scale+addr
   ...
   CAREFUL! the input i is modified and its previous value is lost! */
