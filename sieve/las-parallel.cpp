...
struct las_parallel_desc::helper {
...
#endif
    int nsubjobs_per_cpu_binding_zone;
    int nthreads_per_subjob;
    ...
    */
    ...
    bool is_number(std::string const & s, int & x, std::string::size_type pos = 0) const/*{{{*/
    {
    ...
    void parse(const char * desc_c) { /* {{{ */
        ...
        for(size_t k = 3 ; k < tokens.size() ; ++k) {
            if (tokens[k] == "strict") { strict_opt = true; }
            else if (tokens[k] == "loose") { loose_opt = true; }
            else if (tokens[k] == "no-replicate") { norepl_opt = true; }
            else
                throw bad_specification("failed at option ", tokens[k]);
        }
   ...
   int interpret_binding_specifier() {/*{{{*/
        ...
        */
       if (is_number(memory_binding_specifier_string, memory_binding_size))
       ...
       if (!multiplier_string.empty()) {
           int x;
           bool t = is_number(multiplier_string, x);
           ASSERT_ALWAYS(t);
           objsize *= x;
       }
       ...
       if (!divisor_string.empty()) {
           int x;
           bool t = is_number(divisor_string, x);
           ASSERT_ALWAYS(t);
           if (objsize % x) {
               std::ostringstream os;
       ...
       return cpu_binding_size = memory_binding_size = objsize;
#else
   ...
   int interpret_njobs_specifier() {/*{{{*/
       if (is_number(jobs_within_cpu_binding_string, nsubjobs_per_cpu_binding_zone))
   ...
   int interpret_nthreads_specifier() {/*{{{*/
       if (is_number(threads_per_job_string, nthreads_per_subjob))
...
[binding level],[number of jobs with this binding],[threads per job][,modifiers]

All three main items may be set as integers. However this probably not
the most useful, and certainly not the most portable way to set them.
    ...
    : jobram(jobram_arg)
{
    ...
    if (desc_c == NULL) {/*{{{*/
        /* use a default. For now we're content with the default that is
         * set in the class description itself.
         */
        return;
    }/*}}}*/
    ...
    ncpu_binding_zones_per_memory_binding_zone = 1;
