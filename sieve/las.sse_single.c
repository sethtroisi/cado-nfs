...
// This option might be more precise for norm computation.
//#define SSE_USE_DOUBLE_PREC
    ...
    for (; j < lastj; ++j)
      {
...
#else  // use single prec
        union { __v4sf dble;
            __v4si intg;
            struct {uint32_t y0; uint32_t y1;
                uint32_t y2; uint32_t y3;} intuple;
        } y_vec;
        float sgi = (float)gi;
        uint32_t mask32 = mask;
        float zzz = (float)(zx->z);
        ...
        if ((j & 1) == 1) {
            // in spite of the appearance, only the low word gives the shift
            // count. The high word is ignored.
            __v4sf z_vec = { zzz, zzz+sgi, zzz+2*sgi, zzz+3*sgi };
            __v4sf gi_vec = { 4*sgi, 4*sgi, 4*sgi, 4*sgi };

            for (i = 0; i < halfI>>1; ++i) {
                y_vec.dble = z_vec;
                y_vec.intg -= cst_vec;
                y_vec.intg = _mm_srl_epi32(y_vec.intg, shift_value);
                y_vec.intg &= mask_vec;
                //            *S++ = si->S_rat[((uint32_t *)(&y_vec.intg))[0]];
                //             *S++ = si->S_rat[((uint32_t *)(&y_vec.intg))[2]];
                *S++ = si->S_rat[y_vec.intuple.y0];
                *S++ = si->S_rat[y_vec.intuple.y1];
                *S++ = si->S_rat[y_vec.intuple.y2];
                *S++ = si->S_rat[y_vec.intuple.y3];
                z_vec += gi_vec;
            }
        } else { // skip when i and j are both even
            // in spite of the appearance, only the low word gives the shift
            // count. The high word is ignored.
            __v4sf z_vec = { zzz+sgi, zzz+3*sgi, zzz+5*sgi, zzz+7*sgi };
            __v4sf gi_vec = { 8*sgi, 8*sgi, 8*sgi, 8*sgi };

            for (i = 0; i < halfI>>1; i+=2) {
                y_vec.dble = z_vec;
                y_vec.intg -= cst_vec;
                y_vec.intg = _mm_srl_epi32(y_vec.intg, shift_value);
                y_vec.intg &= mask_vec;
...
static inline void
init_fpoly_v4sf(__v4sf *F, const double *f, const int deg)
{
    int i;
    for (i = 0; i <= deg; ++i) {
        float fi = (float) f[i];
        __v4sf tmp = { fi, fi, fi, fi };
        F[i] = tmp;
    }
}

static inline __v4sf
fpoly_eval_v4sf(const __v4sf *f, const __v4sf x, int d)
{
    __v4sf r;
    r = f[d--];
    for (; d>=0; --d)
...
#ifdef SSE_NORM_INIT
      __v4sf u_vec[d+1];
      init_fpoly_v4sf(u_vec, u, d);
      float i0[3] = {0.0, 0.0, 0.0} ;
      unsigned char *S_ptr0[3] = { NULL, NULL, NULL};
      int cpt = 0;
#endif
      /* now compute norms */
...
//#if 1
              ...
              *alg_S = n;
              ...
              __v4si shift_value = { (uint32_t)(23 - l), (uint32_t)(23 - l),
                  (uint32_t)(23 - l), (uint32_t)(23 - l) };
              report++;      
              if (cpt < 3) {  // wait until we have 4 computations to do
                  i0[cpt] = (float)i;
                  S_ptr0[cpt] = alg_S;
                  cpt++;
              } else {
                  ASSERT(cpt == 3);
                  cpt = 0;
                  __v4sf i_vec = {i0[0], i0[1], i0[2], (float)i};
                  union { __v4sf dble;
                      __v4si intg;
                      struct {uint32_t y0; uint32_t y1; 
                              uint32_t y2; uint32_t y3;} intuple;
                  } fi_vec;
                  fi_vec.dble = fpoly_eval_v4sf(u_vec, i_vec, d);
                  fi_vec.intg -= cst_vec;
                  fi_vec.intg = _mm_srl_epi32(fi_vec.intg, shift_value);
                  fi_vec.intg &= mask_vec;
                  *S_ptr0[0] = T[fi_vec.intuple.y0];
                  *S_ptr0[1] = T[fi_vec.intuple.y1];
                  *S_ptr0[2] = T[fi_vec.intuple.y2];
                  *alg_S =     T[fi_vec.intuple.y3];
              }
#endif
            }
          else
