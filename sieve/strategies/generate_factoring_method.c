 ...
 */
double*
distribution_prime_number(int min, int max)
{
  int len = max-min;
  ASSERT (len >= 0);
...
void
generate_prime_factor(mpz_t res, gmp_randstate_t state, int lenFact)
{
  mpz_t min;
  mpz_t max;
  mpz_t diff;
				     ...
				     int lenFact1_max, int lenFactall)
{
  ...
  int precision = 10000;//10000 to consider approximation of distribution
...
facul_strategy_t *
generate_fm (int method, int curve, unsigned long B1, unsigned long B2)
{
...
*/
void
bench_proba (gmp_randstate_t state, tabular_fm_t* fm, int len_p_min)
{
  ...
  for (int i = 0; i < len; i++)
      ...
      do
	{
	  ...
	  ind_proba++;
	} while ((proba[ind_proba-1] - BENCH_MIN_PROBA) > EPSILON_DBL && ind_proba < p_max);
...
*/
void
bench_time (gmp_randstate_t state, tabular_fm_t* fm)
{
  unsigned long* param;
  fm_t* elem;  
  int len = fm->index;//number of methods!
...
static double*
sub_routine_bench_proba_cost_interval (facul_strategy_t *strategy, 
...
*/
double*
choice_parameters (int method, int len_p_min)
{
...
static tabular_fm_t*
bench_proba_cost_interval (int method, int curve, gmp_randstate_t state,
			     int len_p_min, int len_p_max, int len_n, 
			     double* param_region) 
{
...
*/
tabular_fm_t*
generate_factoring_methods_mc (gmp_randstate_t state, int len_p_min,
			       int len_p_max, int len_n, int method, 
			       int curve, int opt_ch, double* param_sieve)
{
...
tabular_fm_t*
generate_factoring_methods (gmp_randstate_t state, int len_p_min, 
			    ...
			    double* param_sieve)
{
  ...
  while (method <= NB_METHOD && curve < NB_CURVE)
	...
	(state, len_p_min, len_p_max, len_n, method, curve, opt_ch, param_sieve);
 ...
 */
tabular_fm_t*
convex_hull_from_file (FILE* file_in, FILE* file_out)
{
...
tabular_fm_t*
filtering (tabular_fm_t* fm, int len_p_min, int nb_prime_nb)
{
  int len = fm->index;
  //create compromis
  ...
  curve_near[0] = 0;
  curve_near[1] = 0;
  double * disp = distribution_prime_number (len_p_min, len_p_min + nb_prime_nb);
  ...
  do
      ...
      for (int i = 0; i < len; i++)
	{
	  ...
	  int nb_min = (el0->len_proba < el0->len_proba)?el0->len_proba:el1->len_proba;
  ...
  //filter ECM METHODS:: take advantage that ECM can be repeat over several curves!!!!
  for (int i = 0; i < len; i++)
    {
	      ...
	      if (method_j[0] == EC_METHOD)
		{
		  ...
		  for (int p = 0; p < nb_min; p++)
		      ...
		      else
			{
		      ...
		      /* printf ("t2 = %f, t1 = %f\n", t2, t1); */
		      /* printf("ratio = %d, %f, %f\n",ratio, pow(1-p1, ratio), 1-p2); */
		      /* printf ("p1 = %f, t1= %f, p2 = %f, t2 = %f\n", p1, t1, p2, t2); */
		      /* getchar(); */
  ...
  int nb_zero = 0;
  printf ("filtering_*** represente les données collectés pour les méthodes qui n'ont pas été filtrées!!!\n");
  ...
  for(int i = 0; i < len; i++)
      ...
      if (!tabular_fm_is_zero(fm, i) )
	{
	  ...
	  fprintf (select_fm, "#filtering fm[%d]: [%d, ...]\n # len_p\t proba\t tps \t compromis\n", i, len_p_min);
	  
	  for (int k = 0; k < nb_prime; k++)
	    fprintf (select_fm, "%d \t %f\t %f\t %f\n", len_p_min+k, elem->proba[k], elem->time[k], compromis[i][k]);
	  /* fclose (select_fm); */
	}
      else if (nb_zero == 0)
      	{
...
tabular_point_t*
convert_tab_point_to_tab_fm (tabular_fm_t* t)
{
...
tabular_fm_t*
convert_tab_fm_to_tab_point (tabular_point_t* t, tabular_fm_t* init)
{
  tabular_fm_t* res = tabular_fm_create ();
