...
decomp=$4 # say '/localdisk/trichard/results/decomp_cofactor/decomp_tmp/'
#file which contains our factoring methods (PM1, PP1, EC)!
methods=$5 # say 'All_methods'
#directory where the output file will be stored, because this binary
#generates a file for each r.
out=$6 # say 'res_precompt_st/'

let "r0=0"
while [ $r0 -le $mfb0 ]; do
pathname_decomp=$decomp"/decomp_"$lim0"_"$r0
./gst -gst_r -lim0 $lim0 -lpb0 $lpb0 -r0 $r0 -in $methods -decomp $pathname_decomp -out $out &
let "r0 = r0 + 1"
done
