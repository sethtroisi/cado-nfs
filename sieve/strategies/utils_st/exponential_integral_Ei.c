...
////////////////////////////////////////////////////////////////////////////////
// Compute Number of prime between two elemts x1 and x2!
////////////////////////////////////////////////////////////////////////////////

long double 
xExponential_Integral_Ei_Ej( long double x1, long double x2 )
{
   if ( x1 < 50.0 && x2 < 50.0 && x1 > 6.8 && x2 > 6.8) 
     return Argument_Addition_Series_Ei_Ej(x1,x2);
   return xExponential_Integral_Ei(x2) - xExponential_Integral_Ei(x1);
}

double 
Exponential_Integral_Ei_Ej( double x1, double x2 )
{
  return (double) xExponential_Integral_Ei_Ej ((long double) x1, (long double) x2); 
}

long double 
Argument_Addition_Series_Ei_Ej(long double x1, long double x2)
{
  static long double ei[] = {
    1.915047433355013959531e2L,  4.403798995348382689974e2L,
    1.037878290717089587658e3L,  2.492228976241877759138e3L,
  ...
  int k2 = (int) (x2 + 0.5);
  int j = 0;
  long double xx1 = (long double) k1;
  long double xx2 = (long double) k2;
  long double dx1 = x1 - xx1;
  long double dx2 = x2 - xx2;
  long double xxj1 = xx1;
  long double xxj2 = xx2;
  long double edx1 = expl(dx1);
  long double edx2 = expl(dx2);
  long double Sm1 = 1.0L;
  long double Sm2 = 1.0L;
  long double Sn1 = (edx1 - 1.0L) / xxj1;
  long double Sn2 = (edx2 - 1.0L) / xxj2;
  long double term1 = DBL_MAX;
  long double term2 = DBL_MAX;
  long double factorial = 1.0L;
  long double dxj1 = 1.0L;
  long double dxj2 = 1.0L;
  long double tmp1 = Sn1* expl(xx1)+ ei[k1-7];
  long double tmp2 = Sn2* expl(xx2)+ ei[k2-7];
  while (fabsl(term1) > epsilon * fabsl(Sn1) && fabsl(term2) > epsilon * fabsl(Sn2)) 
    {
      j++;
      factorial *= (long double) j;
      xxj1 *= xx1;
      ...
      Sm1 += (dxj1 / factorial);
      Sm2 += (dxj2 / factorial);
      term1 =  ( factorial * (edx1 * Sm1 - 1.0L) ) / xxj1;
      term2 =  ( factorial * (edx2 * Sm2 - 1.0L) ) / xxj2;
      Sn1 += term1 ;
      Sn2 += term2 ;
      tmp1 = Sn1* expl(xx1)+ ei[k1-7];
      tmp2 = Sn2* expl(xx2)+ ei[k2-7];
...
long double 
Continued_Fraction_Ei_Ej (long double x1, long double x2 )
{
  //x1
  long double Am1 = 1.0L;
  long double A1 = 0.0L;
  long double Bm1 = 0.0L;
  long double B1 = 1.0L;
  long double a1 = expl(x1);
  long double b1 = -x1 + 1.0L;
  long double Ap1 = b1 * A1 + a1 * Am1;
  long double Bp1 = b1 * B1 + a1 * Bm1;

  //x2
  long double Am2 = 1.0L;
  long double A2 = 0.0L;
  long double Bm2 = 0.0L;
  long double B2 = 1.0L;
  long double a2 = expl(x2);
  long double b2 = -x2 + 1.0L;
  long double Ap2 = b2 * A2 + a2 * Am2;
  long double Bp2 = b2 * B2 + a2 * Bm2;
  
  //  long double tmp2 = Ap
  long double tmpABp = fabs((-Ap2 / Bp2) - (-Ap1 / Bp1));
  long double tmpAB = fabs((-A2 / B2) - (-A1 / B1));
  int j = 1;
  a1 = 1.0L;
  a2 = 1.0L;
  while ( tmpAB > epsilon * tmpABp)
    {
      //x1
      if ( fabsl(Bp1) > 1.0L) {
	Am1 = A1 / Bp1;
	...
	B2 = Bp2;
      ...
      j += 1;
      tmpABp = fabs((-Ap2 / Bp2) - (-Ap1 / Bp1));
      tmpAB = fabs((-A2 / B2) - (-A1 / B1));
  ...
  /* //test */
  /* double t0 = pow(2,60)+(rand() % 5096); */
  /* double t01 = t0+200000; */
  /* double tmp0 = Exponential_Integral_Ei(log(t0)); */
  /* double tmp01 = Exponential_Integral_Ei(log(t01)); */
  /* long double tmp00 = Argument_Addition_Series_Ei_Ej(logl(t0), logl(t01)); */
  /* printf ("tmp %f\t log: %f, %f\n", tmp01-tmp0, log(t01),log(t0));  */
  /* printf ("tmp %Lf\n", tmp00);  */
  /* unsigned long rv = real_value (t0, t01); */
  /* printf ("rv = %lu\n", rv); */
