...
void
tabular_fm_free (tabular_fm_t* t)
{
...
int
tabular_fm_get_index (tabular_fm_t* t)
{
  return t->index;
}
...
void
tabular_fm_add_fm (tabular_fm_t *t, fm_t* fm)
{
  tabular_fm_set_fm_index (t, fm, t->index);
}

void
tabular_fm_add (tabular_fm_t *t, unsigned long* method, int len_method, double* proba,int len_proba, double* time, int len_time, int len_p_min)
{
  tabular_fm_set_index (t, method, len_method, proba, len_proba, time, len_time, len_p_min, t->index);
}

void
tabular_fm_set_fm_index (tabular_fm_t *t, fm_t* fm, int ind)
{
  tabular_fm_set_index (t, fm->method, fm->len_method, fm->proba, fm->len_proba, fm->time, fm->len_time, fm->len_p_min, ind);
}

void
tabular_fm_set_index (tabular_fm_t *t, unsigned long* method, int len_method, double* proba, int len_proba, double* time, int len_time, int len_p_min, int ind)
{
  if (ind >= t->size)
...
void
tabular_fm_concat(tabular_fm_t* t1, tabular_fm_t* t2)
{
  int len = t2->index;
  for (int i=0; i < len; i++)
...
tabular_fm_t*
extract_fm_method (tabular_fm_t* t, int method, int curve)
{
...
int
tab_fm_is_exist (tabular_fm_t* c, fm_t* f)
{
  int len = c->index;
  for (int i =0; i < len; i++)
    {
...
int
tabular_fm_print (tabular_fm_t* t)
{
  return tabular_fm_fprint (stdout, t);
}

int
tabular_fm_fprint (FILE* file, tabular_fm_t* t)
{
  int len = t->index;
  for (int i=0; i < len; i++)
    {
...
static int
is_elem (const char c)
{
  return (c >= 48 && c <= 57) || c == '|';
}

static void
next_elem (FILE* file, char* current_char)
{
...
static fm_t*
sub_routine_fm_fscanf (FILE* file, char* current_char)
{
  fm_t* fm = fm_create();
...
int 
fm_fscan (FILE* file, tabular_fm_t* res)
{ 
  if (file == NULL)
    return -1;
...
void
fm_swap (tabular_fm_t* t, int index1, int index2)
{
  fm_t* c = t->tab[index1];
...
static void
tabular_fm_sort_rec (tabular_fm_t* t, int begin, int end)
{
  int index_max = begin;
...
void
tabular_fm_sort (tabular_fm_t* t)
{
  int max = t->index;
  while (max > 0)
    {
