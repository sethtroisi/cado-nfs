...
#with -nsm 2 and -t 1
if ! "${SM}" ${args} -out "${TMPDIR}"/sm.2.1 -t 1 -nsm1 2 ; then
  echo "$0: sm binary failed with -t 1 and -nsm1 2. Files remain in ${TMPDIR}"
  exit 1
fi
#with -nsm 2 and -t 2
if ! "${SM}" ${args} -out "${TMPDIR}"/sm.2.2 -t 2 -nsm1 2 ; then
...
if !  diff -b "${TMPDIR}"/sm.2.1 "${TMPDIR}"/sm.2.2 > /dev/null ; then
  echo "$0: Mono-threaded and multi-threaded versions do not match (with -nsm1 2). Files remain in ${TMPDIR}"
  exit 1
fi

if ! diff -b    \
        <(tail -n +2 "${TMPDIR}"/sm.5.1 | cut -d ' ' -f1-2)     \
        <(tail -n +2 "${TMPDIR}"/sm.2.1)                        \
        > /dev/null ; then
