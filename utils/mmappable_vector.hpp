...
#error "Not GNU libstdc++, please expand code"
                ...
                typename std::enable_if<std::is_same<mmap_allocator<T>, A>::value>::type mmap_file(const char * filename, access_mode mode, mmapped_file::offset_type offset, mmapped_file::size_type length) {
#ifdef __GLIBC__
                    A & a(Base::_M_get_Tp_allocator());
#else
#error "Not GNU libstdc++, please expand code"
#endif
                    if (a.s) throw mmap_allocator_exception("already mapped");
                    a = A(filename, mode, offset, length);
                    mmap(length);
                }
                typename std::enable_if<std::is_same<mmap_allocator<T>, A>::value>::type munmap_file() {
#ifdef __GLIBC__
                    A & a(Base::_M_get_Tp_allocator());
#else
#error "Not GNU libstdc++, please expand code"
#endif
                    if (!a.s) throw mmap_allocator_exception("not yet mapped");
                    munmap();
                    a = A();
                }
        };
